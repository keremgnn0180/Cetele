export const generateProfitReport = async ({ summary }) => {
  const rows = (summary.fieldPerformances || []).map((item) => [
    item.isim || "-",
    `${formatNumber(item.donum)} dönüm`,
    formatCurrency(item.revenue),
    formatCurrency(item.expenses),
    formatCurrency(item.profit),
    formatCurrency(item.profitPerDonum),
  ]);

  const html = buildReportHtml({
    reportTitle: "Kâr/Zarar Raporu",
    filters: [
      { label: "Kapsam", value: "Tüm tarlalar ve finansal kayıtlar" },
      { label: "Performans Satırı", value: rows.length },
    ],
    summaryCards: [
    ...
    columns: ["Tarla", "Alan", "Gelir", "Gider", "Net Kâr/Zarar", "Dönüm Başına"],
    rows,
    totals: [
      { label: "Toplam Gelir", value: formatCurrency(summary.totalIncome) },
      { label: "Toplam Gider", value: formatCurrency(summary.totalExpense) },
