...
async function main() {
  ...
  console.log(
    "Database initialized. Engine:",
    isWasm ? "sql.js (WASM fallback)" : "better-sqlite3",
  );
  ...
  database.run(`INSERT INTO tarlalar (isim, donum, konum) VALUES (?, ?, ?)`, [
    uniqueTarlaName,
    20,
    "Adana/Seyhan",
  ]);
  const tarlaRow = database.query("SELECT id FROM tarlalar WHERE isim = ?", [
    uniqueTarlaName,
  ]);
  const tarlaId = tarlaRow[0].id;
  const uniqueUrunName = `Mısır ${Date.now()}`;
  database.run(`INSERT INTO urunler (isim, kategori) VALUES (?, ?)`, [
    uniqueUrunName,
    "Tahıl",
  ]);
  const urunRow = database.query("SELECT id FROM urunler WHERE isim = ?", [
    uniqueUrunName,
  ]);
  const urunId = urunRow[0].id;
  // Masraf (Gübre)
  database.run(
    `INSERT INTO masraflar (tarla_id, kategori, urun_adi, miktar, birim, birim_fiyat, tutar, tarih, aciklama) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      tarlaId,
      "Gübre",
      "",
  ...
  database.run(
    ...
    [tarlaId, urunId, 8, "ton", 9500, 8 * 9500, new Date().toISOString(), ""],
  );
  // Verify inserts via queries
  const tarlalar = database.query("SELECT * FROM tarlalar WHERE id = ?", [
    tarlaId,
  ]);
  const urunler = database.query("SELECT * FROM urunler WHERE id = ?", [
    urunId,
  ]);
  const masraflar = database.query(
    "SELECT * FROM masraflar WHERE tarla_id = ?",
    [tarlaId],
  );
  const hasatlar = database.query("SELECT * FROM hasatlar WHERE tarla_id = ?", [
    tarlaId,
  ]);
