...
async function initDatabase(appDataPath) {
  ...
  try {
    ...
    console.warn(
      "better-sqlite3 yÃ¼klenemedi. sql.js WASM motoruna geÃ§iÅŸ yapÄ±lÄ±yor...",
      err.message,
    );
    ...
    try {
      ...
      console.log(
        "Database engine:",
        isWasm ? "sql.js fallback" : "better-sqlite3",
      );
      applyPragmas();
      createTables();
      migrateTables();
      ...
      console.error(
        "KRÄ°TÄ°K HATA: HiÃ§bir veritabanÄ± motoru baÅŸlatÄ±lamadÄ±!",
        wasmErr.message,
      );
      throw wasmErr;
    }
...
function migrateTables() {
  ...
  try {
    const expenseCols = query("PRAGMA table_info(masraflar);").map(
      (c) => c.name,
    );
    ...
    console.log(
      "Masraflar migration hatasÄ± (gÃ¶z ardÄ± edilebilir):",
      e.message,
    );
  }
}
...
function runAutoDailyBackup() {
  ...
  try {
    ...
    const alreadyBackedUpToday = files.some((file) =>
      file.startsWith(`cetele-${todayStr}`),
    );
    ...
    if (!alreadyBackedUpToday) {
      ...
      console.log(
        "GÃ¼nlÃ¼k otomatik yedek baÅŸarÄ±yla oluÅŸturuldu:",
        backupPath,
      );
      ...
      console.log(
        "BugÃ¼n iÃ§in otomatik yedek zaten mevcut, yeni yedek alÄ±nmadÄ±.",
      );
    }
  } catch (err) {
...
async function importBackup(srcPath) {
  try {
    ...
    console.log(
      "Yedek baÅŸarÄ±yla iÃ§e aktarÄ±ldÄ± ve veritabanÄ± yeniden baÄŸlandÄ±.",
    );
    return { success: true };
  } catch (err) {
...
function healthCheck() {
  const rows = query("SELECT name FROM sqlite_master WHERE type = ? LIMIT 1", [
    "table",
  ]);
  return {
    ok: true,
