...
const isDev =
  !isE2E && (process.env.NODE_ENV === "development" || !app.isPackaged);
...
function recoverLegacyProfile(appDataPath) {
  ...
  for (const legacyPath of legacyCandidates) {
    ...
    if (fs.existsSync(legacyDbPath)) {
      ...
      const shouldRecoverDb =
        !hasCurrentDb ||
        (currentDbSize < 32768 && legacyDbSize > currentDbSize);
...
async function bootstrap() {
  ...
  const securityManager = new SecurityManager({
    isDev,
    logger: loggerService.child("security"),
  });
  ...
  await updateService
    .check("startup")
    .catch((error) =>
      logger.warn("Startup update check skipped:", error.message),
    );
}
