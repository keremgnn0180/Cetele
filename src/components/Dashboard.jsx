...
function Dashboard({ setActiveTab }) {
  ...
  const [welcomeName, setWelcomeName] = useState(
    () => localStorage.getItem(USER_NAME_KEY) || "Ciftci",
  );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("cetele:user-name-updated", handleNameUpdate);
  }, []);
      ...
      <div className="stats-grid stats-grid-4">
        <div className="stat-card clean">
          ...
          <div className="stat-copy">
            ...
            <small>
              Toplam {stats.toplamDonum.toLocaleString("tr-TR")} donum
            </small>
          </div>
        </div>
        <div className="stat-card clean">
        ...
        <div className="card">
          ...
          <div className="quick-grid">
            <button
              ...
              onClick={() => setActiveTab("tarlalar")}
            >
              <Map className="quick-item-icon" size={22} />
              Tarla Ekle
            </button>
            <button
              className="quick-item"
              onClick={() => setActiveTab("masraflar")}
            >
              <DollarSign className="quick-item-icon" size={22} />
              Masraf Ekle
            </button>
            <button className="quick-item" onClick={() => setActiveTab("ekim")}>
              <CalendarDays className="quick-item-icon" size={22} />
              Ekim Kaydi
            </button>
            <button
              className="quick-item"
              onClick={() => setActiveTab("hasat")}
            >
              <TrendingUp className="quick-item-icon" size={22} />
              Hasat Kaydi
            </button>
            <button
              className="quick-item"
              onClick={() => setActiveTab("raporlar")}
            >
              <FileText className="quick-item-icon" size={22} />
              Raporlar
            </button>
            <button
              className="quick-item"
            ...
            >
              <Database className="quick-item-icon" size={22} />
              Yedekleme
            </button>
          </div>
          <button
