...
function Hasat() {
  ...
  const handleDelete = async (id) => {
    if (
      confirm("Bu hasat ve satış kaydını silmek istediğinize emin misiniz?")
    ) {
      try {
  ...
  const filteredHasatlar = hasatlar.filter((h) => {
    // Tarla filtresi
    if (filterTarla !== "HEPSİ" && h.tarla_id.toString() !== filterTarla)
      return false;

    // Ürün filtresi
    if (filterUrun !== "HEPSİ" && h.urun_id.toString() !== filterUrun)
      return false;
  ...
  const calculatedTotal =
    parseFloat(form.miktar) * parseFloat(form.birim_satis_fiyati);
        ...
        <button
          ...
          onClick={() => {
            if (tarlalar.length === 0 || urunler.length === 0) {
              alert(
                "Hasat kaydı eklemeden önce en az bir Tarla ve bir Ürün tanımlamalısınız!",
              );
            } else {
              setShowModal(true);
            }
      ...
      {filteredHasatlar.length === 0 ? (
        <div className="card empty-state">
          ...
          <p style={{ marginTop: "8px" }}>
            Filtrelerinize uygun veya kaydedilmiş bir satış kaydı
            bulunmamaktadır. &quot;Yeni Hasat / Satış Ekle&quot; ile ekleme
            yapın.
          </p>
        </div>
      ) : (
            ...
            <tbody>
              {filteredHasatlar.map((hasat) => (
                <tr key={hasat.id}>
                  ...
                  <td>
                    {new Date(hasat.tarih).toLocaleDateString("tr-TR", {
                      day: "numeric",
                      month: "long",
                      year: "numeric",
                    })}
              ...
              <div className="form-row" style={{ marginTop: "10px" }}>
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, birim_satis_fiyati: e.target.value })
                    }
                    required
                  />
                </div>
