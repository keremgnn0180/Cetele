...
function Masraflar() {
  ...
  const isTarlaIsciligi =
    form.urun_adi.trim().toLocaleLowerCase("tr-TR") === "tarla işçiliği";
  ...
  const varietyOptions = useMemo(() => {
    if (!form.gubre_marka || !form.gubre_turu || isManualBrand || isManualType)
      return [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.kategori === "Gübre") {
        ...
        fertilizerFields.marka = isManualBrand
          ? newBrand.trim()
      ...
      const fertilizerName = ["Gübre", "İlaç", "Tohum"].includes(form.kategori)
        ? form.kategori === "Gübre"
          ? [
              ...
              fertilizerFields.tur,
              fertilizerFields.cesit,
            ]
              .filter(Boolean)
              .join(" - ")
              .trim()
      ...
      const payload = {
        ...
        gubre_marka: ["Gübre", "İlaç", "Tohum"].includes(form.kategori)
          ? fertilizerFields.marka
          : null,
  ...
  const handleEdit = (masraf) => {
    ...
    setForm({
      ...
      urun_adi:
        ["Gübre", "İlaç", "Tohum"].includes(masraf.kategori) &&
        masraf.gubre_marka
  ...
  const filteredMasraflar = masraflar.filter((m) => {
    if (filterTarla !== "HEPSİ") {
      ...
      if (filterTarla !== "GENEL" && m.tarla_id?.toString() !== filterTarla)
        return false;
    }
    if (filterKategori !== "HEPSİ" && m.kategori !== filterKategori)
      return false;
  ...
  const calculatedTotal =
    parseFloat(form.miktar) * parseFloat(form.birim_fiyat);
                ...
                return (
                  <tr key={masraf.id}>
                    ...
                    <td>
                      <span
                        className={
                          isCustomBadge ? "badge" : `badge ${colorStyle}`
                        }
