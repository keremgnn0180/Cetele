...
function Ayarlar() {
      ...
      unsubs = [
        ...
        window.api.onUpdateNotAvailable((payload) => {
          setUpdateInfo((prev) => ({
            ...prev,
            status: "Güncel",
            lastCheck: payload?.checkedAt || prev.lastCheck,
          }));
          setUpdating(false);
        }),
        window.api.onUpdateError((payload) => {
          setUpdateInfo((prev) => ({
            ...prev,
            status: `Hata: ${payload?.message || "Bilinmeyen hata"}`,
          }));
          setUpdating(false);
        }),
      ];
  ...
  const handleSaveProfile = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("cetele:user-name-updated", { detail: trimmed }),
    );
    setStatus({
      type: "success",
  ...
  const handleCheckUpdates = async () => {
    ...
    try {
      ...
      if (!res?.ok) {
        setUpdateInfo((prev) => ({
          ...prev,
          status: `Hata: ${res?.error || "Kontrol başarısız"}`,
        }));
        setUpdating(false);
      } else {
        setUpdateInfo((prev) => ({
  ...
  const handleRestore = async () => {
    ...
    if (
      confirm(
        "DİKKAT: Yeni bir yedek yüklediğinizde mevcut tüm verileriniz silinecek ve yedekteki veriler yazılacaktır! Geri yükleme işlemine devam etmek istiyor musunuz?",
      )
    ) {
      setLoading(true);
      try {
    ...
    <div style={{ maxWidth: "800px", margin: "0 auto" }}>
      ...
      >
        ...
        <div>
          ...
          >
            Uygulama tüm verilerinizi bilgisayarınızda yerel ve yüksek
            performanslı bir **SQLite** veritabanında saklar. Verileriniz hiçbir
            bulut sunucusuna gönderilmez, tamamen sizin kontrolünüzdedir.
            Verilerinizi güvenceye almak için aşağıdaki yedekleme araçlarını
            kullanabilirsiniz.
          </p>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            Mevcut tarlalarınızı, ürünlerinizi, ekim, masraf ve hasat
            kayıtlarınızı tek bir yedek dosyası (`.db`) olarak bilgisayarınıza
            indirin. Bilgisayar değişimi veya veri güvenliği için düzenli yedek
            almanız önerilir.
          </p>
          <button
          ...
          >
            <Download size={18} />
            <span>
              {loading ? "Yedekleniyor..." : "Yedek Dosyasını Kaydet"}
            </span>
          </button>
        </div>
        ...
        >
          ...
          >
            Daha önce almış olduğunuz bir yedek dosyasını (`.db`, `.sqlite` veya
            `.bak`) uygulamaya yükleyerek verilerinizi geri getirin. Bu işlem
            güncel kayıtlarınızı tamamen silecektir.
          </p>
          <button
      ...
      >
        ...
        <div>
          ...
          >
            Otomatik Arka Plan Yedeklemesi Aktif
          </h4>
          <p
            style={{
              fontSize: "0.88rem",
          ...
          >
            Siz yedek almayı unutsanız bile, uygulamamız her açılışta
            veritabanınızı otomatik yedekler ve `backups` klasöründe tarih
            damgalı `cetele-YYYY-AA-GG-SS-DD.db` dosyaları oluşturur.
          </p>
        </div>
      </div>
      ...
      <div className="card" style={{ marginTop: "24px" }}>
        <h4 style={{ fontSize: "1.1rem", marginBottom: "14px" }}>
          Güncellemeler
        </h4>
        <div
          style={{
        ...
        >
          <div>
            <strong>Mevcut sürüm:</strong>{" "}
            {updateInfo.currentVersion || "v1.0.0"}
          </div>
          <div>
            <strong>Son kontrol:</strong>{" "}
            {updateInfo.lastCheck
              ? new Date(updateInfo.lastCheck).toLocaleString("tr-TR")
              : "Henüz yapılmadı"}
          </div>
          <div>
            <strong>Durum:</strong> {updateInfo.status || "Bilinmiyor"}
          </div>
          {updateInfo.progress > 0 && updateInfo.progress < 100 && (
            <div>
              <strong>İndirme:</strong> %{Math.round(updateInfo.progress)}
            </div>
          )}
        </div>
        <button
